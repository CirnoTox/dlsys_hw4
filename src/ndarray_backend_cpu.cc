...
#define TILE 8
    ...
    // SELF DEFINED
    bool nextIndex(std::vector<size_t> &indices, std::vector<int32_t> &shape, long itIndex)
    {
      indices[itIndex]++;
    ...
    // SELF DEFINED
    size_t getNDIndex(long sz, size_t offset, std::vector<int32_t> &strides, std::vector<size_t> &indices)
    {
      size_t rawIndex = 0;
